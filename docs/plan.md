...
- **At the CLI and the service**, once `library-config` lands: a
  `--lib 2=PATH/ws,EXTENDED` flag, or the same in `sw-apl.toml`. Until
  then, `scripts/lib-shim.sh` makes a library directory whose `ws/lib1`
  is a link to this repository's `ws/`, so `sw-apl --library
  target/shim` lists and loads these workspaces as library 1. The
  samples say `)LOAD 1 NAME` while the shim is in use and `)LOAD 2
  NAME` after; the switch rebases every baseline once, with that
  reason.
- **In the browser**, once `browser-libraries` lands: this repository
  publishes `ws/` on GitHub Pages, whose responses carry
  `Access-Control-Allow-Origin: *`, with an index file beside the
  workspaces that lists them. The index is `ws/library.json`:
  ...
  lines, tracked, and gated to be current. The field names are a
  proposal until sw-apl's step settles them; changing them is one
  script.
...
|---|---|---|
| `)LIB 2`, `)LOAD 2 NAME` from a configured directory | `library-config` | Shim until it lands |
| `)LIBS` naming this library | `library-config` | Nothing to do here |
| Libraries by URL in the browser, an index file | `browser-libraries` | `library.json` and Pages, Phase 0 |
| `)HELP`, `)DIALECT` for COURSE's opening lesson | `dialect`, `help` | Lesson text waits for the reply format |
...
Runs when sw-apl's `library-config` and `browser-libraries` steps
have landed, and is re-planned then against what they built.

1. `lib-cli` -- the samples switch from the shim's `)LOAD 1` to
   `)LOAD 2`, every baseline rebased with that one reason; `)LIBS`
   shows EXTENDED; the README's "Use it" says exactly what to type.
2. `lib-browser` -- `library.json` in the shape sw-apl's page reads;
   GitHub Pages serving `ws/`; a check that the published demo lists
   and loads COURSE in each mode. **Milestone 5:** `)LOAD 2 COURSE`
   and `START` in the browser.
3. `lib-course-opening` -- lesson 1's `)DIALECT`, `)LIBS` and `)HELP`
   text checked against what sw-apl prints, and the transcript
   re-pinned.
